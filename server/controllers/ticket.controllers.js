...
export const createTicket = async(req, res) => {
    try{
        ...
        return res.status(201).json({ message : "Ticket created and processsing", ticket : newTicket});

    }catch(err){
        return res.status(500).json({ message : "Failed to create a new ticket "});
    }
}
...
export const getTickets = async(req, res) => {
    try{
        ...
        return res.status(500).json({ message : "Failed to get ticket(s) "});
    }
}
